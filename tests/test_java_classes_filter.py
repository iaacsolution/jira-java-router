...
def _cls(name="Foo", package="com.legacy.service", description="", responsibilities=None, dependencies=None):
    return {
        "name": name,
        "package": package,
...
class TestIsIndexable(unittest.TestCase):
    ...
    def test_legit_domain_class(self):
        self.assertTrue(_is_indexable(_cls(
            name="Account",
    ...
    def test_rejected_outside_allowed_package(self):
        self.assertFalse(_is_indexable(_cls(
            name="SecretManager",
    ...
    def test_rejected_aws_key_like_string(self):
        self.assertFalse(_is_indexable(_cls(
            ...
            responsibilities=["utilise la clé AKIAABCDEFGHIJKLMNOP pour l'envoi"],
        )))
